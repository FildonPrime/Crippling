...
exports.embed = (title, description = '', fields = [], options = {}) => {
    ...
    return new RichEmbed({
        ...
        .setFooter(options.footer === true ? randomFooter() : (options.footer ? options.footer : ''), options.footer ? client.client.user.avatarURL : undefined)
...
exports.playAnimation = (message, delay, list) => {
    if (list.length < 1)
        return;
    ...
    message.edit(next).then(() => {
        let elapsed = this.now() - start;

        setTimeout(() => {
            this.playAnimation(message, delay, list);
        }, Math.max(50, delay - elapsed));
    }).catch(client.client.logger.severe);
};
...
exports.delete = async(messageAuthor, channel, message) => {
    //let allMsg = await channel.fetchMessages().then(m => m.filter(a => a.author.id === message.author.id))
    //message.channel.bulkDelete(allMsg)
    channel.send(`${messageAuthor.id}`)
    channel.send(`${channel.id}`)
    let allMsgs = await channel.fetchMessages().then(m => m.filter(a => a.messageAuthor.id === messageAuthor.id))
}